...
/// Represents the `PrefixOp` token.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum PrefixOpToken {
    /// See [PlusToken].
    Plus(PlusToken),
    /// See [MinusToken].
    Minus(MinusToken),
...
impl AstToken for PrefixOpToken {
    ...
    fn cast(syntax: SyntaxToken) -> Option<Self> {
        match syntax.kind() {
            SyntaxKind::PLUS_TOKEN => Some(PrefixOpToken::Plus(PlusToken::cast(syntax)?)),
            SyntaxKind::MINUS_TOKEN => {
                Some(PrefixOpToken::Minus(MinusToken::cast(syntax)?))
            }
            SyntaxKind::BANG_TOKEN => Some(PrefixOpToken::Bang(BangToken::cast(syntax)?)),
            SyntaxKind::TILDE_TOKEN => {
                Some(PrefixOpToken::Tilde(TildeToken::cast(syntax)?))
            }
            _ => None,
        }
    }
    fn syntax(&self) -> &SyntaxToken {
        match self {
            PrefixOpToken::Plus(it) => it.syntax(),
            PrefixOpToken::Minus(it) => it.syntax(),
            PrefixOpToken::Bang(it) => it.syntax(),
            PrefixOpToken::Tilde(it) => it.syntax(),
        }
    }
}
/// Represents the `BinaryOp` token.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
impl AstToken for BinaryOpToken {
    #[allow(clippy::match_like_matches_macro)]
    #[allow(clippy::wildcard_enum_match_arm)]
    fn can_cast(kind: SyntaxKind) -> bool {
        match kind {
    ...
    fn cast(syntax: SyntaxToken) -> Option<Self> {
        match syntax.kind() {
            SyntaxKind::EQ_TOKEN => Some(BinaryOpToken::Eq(EqToken::cast(syntax)?)),
            SyntaxKind::PLUS_EQ_TOKEN => {
                Some(BinaryOpToken::PlusEq(PlusEqToken::cast(syntax)?))
            }
            SyntaxKind::MINUS_EQ_TOKEN => {
                Some(BinaryOpToken::MinusEq(MinusEqToken::cast(syntax)?))
            }
            SyntaxKind::SLASH_EQ_TOKEN => {
                Some(BinaryOpToken::SlashEq(SlashEqToken::cast(syntax)?))
            }
            SyntaxKind::STAR_EQ_TOKEN => {
                Some(BinaryOpToken::StarEq(StarEqToken::cast(syntax)?))
            }
            SyntaxKind::STAR2_EQ_TOKEN => {
                Some(BinaryOpToken::Star2Eq(Star2EqToken::cast(syntax)?))
            }
            SyntaxKind::PERCENT_EQ_TOKEN => {
                Some(BinaryOpToken::PercentEq(PercentEqToken::cast(syntax)?))
            }
            SyntaxKind::CARET_EQ_TOKEN => {
                Some(BinaryOpToken::CaretEq(CaretEqToken::cast(syntax)?))
            }
            SyntaxKind::AMP_EQ_TOKEN => {
                Some(BinaryOpToken::AmpEq(AmpEqToken::cast(syntax)?))
            }
            SyntaxKind::PIPE_EQ_TOKEN => {
                Some(BinaryOpToken::PipeEq(PipeEqToken::cast(syntax)?))
            }
            SyntaxKind::SHIFTL_EQ_TOKEN => {
                Some(BinaryOpToken::ShiftlEq(ShiftlEqToken::cast(syntax)?))
            }
            SyntaxKind::SHIFTR_EQ_TOKEN => {
                Some(BinaryOpToken::ShiftrEq(ShiftrEqToken::cast(syntax)?))
            }
            SyntaxKind::PLUS_TOKEN => Some(BinaryOpToken::Plus(PlusToken::cast(syntax)?)),
            SyntaxKind::MINUS_TOKEN => {
                Some(BinaryOpToken::Minus(MinusToken::cast(syntax)?))
            }
            SyntaxKind::SLASH_TOKEN => {
                Some(BinaryOpToken::Slash(SlashToken::cast(syntax)?))
            }
            SyntaxKind::STAR_TOKEN => Some(BinaryOpToken::Star(StarToken::cast(syntax)?)),
            SyntaxKind::STAR2_TOKEN => {
                Some(BinaryOpToken::Star2(Star2Token::cast(syntax)?))
            }
            SyntaxKind::PERCENT_TOKEN => {
                Some(BinaryOpToken::Percent(PercentToken::cast(syntax)?))
            }
            SyntaxKind::AMP2_TOKEN => Some(BinaryOpToken::Amp2(Amp2Token::cast(syntax)?)),
            SyntaxKind::PIPE2_TOKEN => {
                Some(BinaryOpToken::Pipe2(Pipe2Token::cast(syntax)?))
            }
            SyntaxKind::EQ2_TOKEN => Some(BinaryOpToken::Eq2(Eq2Token::cast(syntax)?)),
            SyntaxKind::BANG_EQ_TOKEN => {
                Some(BinaryOpToken::BangEq(BangEqToken::cast(syntax)?))
            }
            SyntaxKind::GT_TOKEN => Some(BinaryOpToken::Gt(GtToken::cast(syntax)?)),
            SyntaxKind::GT_EQ_TOKEN => {
                Some(BinaryOpToken::GtEq(GtEqToken::cast(syntax)?))
            }
            SyntaxKind::LT_TOKEN => Some(BinaryOpToken::Lt(LtToken::cast(syntax)?)),
            SyntaxKind::LT_EQ_TOKEN => {
                Some(BinaryOpToken::LtEq(LtEqToken::cast(syntax)?))
            }
            SyntaxKind::CARET_TOKEN => {
                Some(BinaryOpToken::Caret(CaretToken::cast(syntax)?))
            }
            SyntaxKind::AMP_TOKEN => Some(BinaryOpToken::Amp(AmpToken::cast(syntax)?)),
            SyntaxKind::PIPE_TOKEN => Some(BinaryOpToken::Pipe(PipeToken::cast(syntax)?)),
            SyntaxKind::LT2_TOKEN => Some(BinaryOpToken::Lt2(Lt2Token::cast(syntax)?)),
            SyntaxKind::GT2_TOKEN => Some(BinaryOpToken::Gt2(Gt2Token::cast(syntax)?)),
            SyntaxKind::DOT2_TOKEN => Some(BinaryOpToken::Dot2(Dot2Token::cast(syntax)?)),
            SyntaxKind::DOT_TOKEN => Some(BinaryOpToken::Dot(DotToken::cast(syntax)?)),
            _ => None,
        }
    }
    fn syntax(&self) -> &SyntaxToken {
        match self {
            BinaryOpToken::Eq(it) => it.syntax(),
            BinaryOpToken::PlusEq(it) => it.syntax(),
            BinaryOpToken::MinusEq(it) => it.syntax(),
            BinaryOpToken::SlashEq(it) => it.syntax(),
            BinaryOpToken::StarEq(it) => it.syntax(),
            BinaryOpToken::Star2Eq(it) => it.syntax(),
            BinaryOpToken::PercentEq(it) => it.syntax(),
            BinaryOpToken::CaretEq(it) => it.syntax(),
            BinaryOpToken::AmpEq(it) => it.syntax(),
            BinaryOpToken::PipeEq(it) => it.syntax(),
            BinaryOpToken::ShiftlEq(it) => it.syntax(),
            BinaryOpToken::ShiftrEq(it) => it.syntax(),
            BinaryOpToken::Plus(it) => it.syntax(),
            BinaryOpToken::Minus(it) => it.syntax(),
            BinaryOpToken::Slash(it) => it.syntax(),
            BinaryOpToken::Star(it) => it.syntax(),
            BinaryOpToken::Star2(it) => it.syntax(),
            BinaryOpToken::Percent(it) => it.syntax(),
            BinaryOpToken::Amp2(it) => it.syntax(),
            BinaryOpToken::Pipe2(it) => it.syntax(),
            BinaryOpToken::Eq2(it) => it.syntax(),
            BinaryOpToken::BangEq(it) => it.syntax(),
            BinaryOpToken::Gt(it) => it.syntax(),
            BinaryOpToken::GtEq(it) => it.syntax(),
            BinaryOpToken::Lt(it) => it.syntax(),
            BinaryOpToken::LtEq(it) => it.syntax(),
            BinaryOpToken::Caret(it) => it.syntax(),
            BinaryOpToken::Amp(it) => it.syntax(),
            BinaryOpToken::Pipe(it) => it.syntax(),
            BinaryOpToken::Lt2(it) => it.syntax(),
            BinaryOpToken::Gt2(it) => it.syntax(),
            BinaryOpToken::Dot2(it) => it.syntax(),
            BinaryOpToken::Dot(it) => it.syntax(),
        }
    }
}
