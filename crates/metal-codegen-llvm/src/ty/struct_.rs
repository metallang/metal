...
use llvm_sys::{core::{LLVMStructCreateNamed, LLVMStructSetBody}, prelude::LLVMTypeRef};
...
impl CodeGenType for Struct {
    fn llvm_type(
        ...
        let full_name = "struct".to_string() + "." + module.name.as_str() + "." + self.name.as_str();
        ...
        unsafe { LLVMStructSetBody(strct, element_types.as_mut_ptr(), self.fields.len().try_into().unwrap(), 0) };
